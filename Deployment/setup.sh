...
echo "Setting current subscription to $SUBSCRIPTION_ID"

az account set --subscription $SUBSCRIPTION_ID

echo "Set current subscription to $SUBSCRIPTION_ID"



# Create a new resource group if it does not exist already, if it exists then check value for USE_EXISTING_RG and based on that either throw error or use the existing RG
if [ $(az group exists --name $RESOURCE_GROUP) == false ]; then
	echo "Creating a new Resource Group $RESOURCE_GROUP"    
	az group create --name $RESOURCE_GROUP --location "$LOCATION"
	echo "Created a new Resource Group $RESOURCE_GROUP"
...
# We are checking if the IoTHub already exists by querying the list of IoT Hubs in current context, it will return a blank array if it does not exist
# Create a new IoT Hub if it does not exist, if it exists then check value for USE_EXISTING_IOT_HUB and based on that either throw error or use the existing IoT Hub
ExistingIoTHub=$(az iot hub list --query "[?name=='$IOTHUB_NAME'].{Name:name}" --output tsv)
if [ -z $ExistingIoTHub ]; then
		echo "Creating a new IoT Hub"
		az iot hub create --name "$IOTHUB_NAME" --sku S1 --resource-group "$RESOURCE_GROUP"
		echo "Created a new IoT hub"
    else
         if [ "$USE_EXISTING_IOT_HUB" == "true" ]; then
                echo "Using existing IoT Hub $IOTHUB_NAME"
        else
                echo "$IOTHUB_NAME already exists"
				exit 1
        fi
fi
...
# Defining the variables outside the if statement so they can be used later
...
echo "Retrieving connection string for device $DEVICE_NAME from Iot Hub $IOTHUB_NAME and updating the IoT Edge service in edge device with this connection string"
...
echo "Updating Config.yaml on edge device with the connection string form IoT Hub"
...
if [ "$IS_THE_SCRIPT_RUNNING_FROM_EDGE_DEVICE" == "true" ]; then
	sudo sed -i -e '/device_connection_string:/ s#\"[^\"][^\"]*\"#\"$ConnectionString\"#' $CONFIG_FILE_PATH
		
	echo "Config.yaml update is complete"
	echo "Restarting IoT Edge service"
	sudo systemctl restart iotedge
	echo "IoT Edge service restart is complete"
else
...
#adding default route in IoT hub
...
# If a pre-generated manifest file is provided by user, we do not need to generate a new manifest file using iotedgedev service
if [ -z $PRE_GENERATED_MANIFEST_FILENAME ]; then
	if [ -e "./${MANIFEST_ENVIRONMENT_VARIABLES_FILENAME}" ] && [ -e "./${MANIFEST_TEMPLATE_NAME}" ]; then
		...
		echo "${MANIFEST_ENVIRONMENT_VARIABLES_FILENAME} and ${MANIFEST_TEMPLATE_NAME} files must be present in current directory"
		exit 1
	fi
else
	if [ "$CREATE_AZURE_MONITOR" == "true" ]; then
		...
		jq --arg valueToBeUpdated "$TELEGRF_AZURE_TENANT_ID" '.modulesContent."$edgeAgent"."properties.desired".modules.telegraf.env.AZURE_TENANT_ID.value = $valueToBeUpdated' test.json > tmp.$$.json && mv tmp.$$.json test.json

		# Updating value for Client ID of Azure Monitor 
		jq --arg valueToBeUpdated "$TELEGRF_AZURE_CLIENT_ID" '.modulesContent."$edgeAgent"."properties.desired".modules.telegraf.env.AZURE_CLIENT_ID.value = $valueToBeUpdated' test.json > tmp.$$.json && mv tmp.$$.json test.json
	
		# Updating value for Client Secret of Azure Monitor 
		jq --arg valueToBeUpdated "$TELEGRF_AZURE_CLIENT_SECRET" '.modulesContent."$edgeAgent"."properties.desired".modules.telegraf.env.AZURE_CLIENT_SECRET.value = $valueToBeUpdated' test.json > tmp.$$.json && mv tmp.$$.json test.json

	fi
fi

echo "Deploying $PRE_GENERATED_MANIFEST_FILENAME manifest file to $DEVICE_NAME Edge device"

#Deploy module manifest file to edge device:
...
echo "Deployed manifest file to IoT Hub"
...
if [ "$PUSH_RESULTS_TO_ADLS" == "true" ]; then
	...
	echo "Creating a custom endpoint in IoT Hub for ADLS" 
	#create a custom-endpoint  to data lake:
	az iot hub routing-endpoint create --resource-group "$RESOURCE_GROUP" --hub-name "$IOTHUB_NAME" --endpoint-name "$ADLS_ENDPOINT_NAME" --endpoint-type azurestoragecontainer --endpoint-resource-group "$RESOURCE_GROUP" --endpoint-subscription-id "$SUBSCRIPTION_ID" --connection-string $storageConnectionString --container-name "$BLOBCONTAINER_NAME" --batch-frequency 100 --chunk-size 100 --ff {iothub}-{partition}-{YYYY}-{MM}-{DD}-{HH}-{mm}
	
	echo "Creating a route in ADLS for ADLS custom endpoint"
	# Create a route for storage endpoint.	
	az iot hub route create   --name "$IOTHUB_ADLS_ROUTENAME"   --hub-name "$IOTHUB_NAME"   --source devicemessages   --resource-group "$RESOURCE_GROUP"   --endpoint-name "$ADLS_ENDPOINT_NAME"   --enabled   --condition "\$connectionDeviceID='$DEVICE_NAME'" 
...
if [ "$PUSH_RESULTS_TO_EVENT_HUB" == "true" ]; then
	...
	echo "Creating a Shared Access Policy for event hub"
	...
	echo "Creating a custom endpoint in IoT hub for event hub"
        #create a endpoint for event hub:

        az iot hub routing-endpoint create --resource-group "$RESOURCE_GROUP" --hub-name "$IOTHUB_NAME"  --endpoint-name "$EVENTHUB_ENDPOINT_NAME" --endpoint-type eventhub  --endpoint-resource-group "$RESOURCE_GROUP" --endpoint-subscription-id "$SUBSCRIPTION_ID"  --connection-string $eventhubconn_string

	echo "Creating a route in IoT hub for event hub custom endpoint"

        #create route for event hub in Iot hub:(Actual condition :"\$twin.moduleid = 'camerastream' and type = 'image'" )
        az iot hub route create   --name "$EVENTHUB_ROUTENAME"   --hub-name "$IOTHUB_NAME"   --source devicemessages   --resource-group "$RESOURCE_GROUP"   --endpoint-name "$EVENTHUB_ENDPOINT_NAME"   --enabled   --condition "true"
