...
// Santa Cruz authentication service URL
...
static const int SantaCruzEyePid = 0x066F;
static const int SantaCruzProtoVersion = 0x200;
...
static libusb_device_handle *open_device(uint16_t vid, uint16_t pid)
{
    ...
    const char *serviceUrl = (desc.bcdDevice == SantaCruzProtoVersion) ? TestServiceUrl : PublicServiceUrl;
            ...
            if (node != NULL && action != NULL)
            {
                ...
                if (strcmp(action, "bind") == 0)
                {
                    ...
                    if (vid != NULL && pid != NULL)
                    {
                        ...
                        if (som_pid == SantaCruzEyePid && strtol(vid, NULL, 16) == Stm32CdcVid && strtol(pid, NULL, 16) == Stm32CdcPid)
                        {
                            ...
                            if (devclass != NULL && strtol(devclass, NULL, 16) == Stm32CdcClass)
                            {
                                ...
                                strncpy(som_node, node, sizeof(som_node));
                                auth_pass = true;
                            }
                        }
