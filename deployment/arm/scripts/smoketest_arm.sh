...
# To check whether the resource name contains suffix
match() {
if [[ "$1" =~ .*"$2".* ]]; then
  echo "1"
fi
...
# To find which resources have the suffix
search() {
local_variable="${1}"
shift
local_array=("${@}")
...
if [ -z "$IOTHUB_NAME" ]; then
    IOTHUB_NAME=$(search "$SUFFIX" "${IOTHUB_LIST[@]}")
fi 
if [ -z "$DEVICE_NAME" ]; then
    DEVICE_NAME="azureEyeEdgeDevice"
fi  
STORAGE_ACCOUNT_NAME=$(search "$SUFFIX" "${STORAGE_ACCOUNT_LIST[@]}")
APP_SERVICE_PLAN_NAME=$(search "$SUFFIX" "${APP_SERVICE_PLAN_LIST[@]}")
WEBAPP_NAME=$(search "$SUFFIX" "${WEB_APP_LIST[@]}")

mapfile -t DEPLOYMENT_NAME_LIST < <(az iot edge deployment list --hub-name "$IOTHUB_NAME" --query "[?id].{Id:id}" --output tsv)
DEPLOYMENT_NAME=$(search "$SUFFIX" "${DEPLOYMENT_NAME_LIST[@]}")
...
az network nsg rule create --name "AllowSSH" --nsg-name "default-NSG" --priority 100 --resource-group "$RESOURCE_GROUP_DEVICE" --destination-port-ranges 22 --source-address-prefixes "$CURRENT_IP_ADDRESS" --output "none"
